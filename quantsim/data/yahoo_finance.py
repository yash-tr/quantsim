...
class YahooFinanceDataHandler(DataHandler):
    ...
    def __init__(self,
        ...
        if not any(not df.empty for df in self.symbol_data.values()):
             print(f"Warning: YahooFinanceDataHandler failed to fetch any data for symbols: {self.symbols} "
    ...
    def _fetch_one_symbol(self, symbol: str) -> pd.DataFrame:
        ...
        """
        try:
            print(f"YahooFinanceDataHandler: Fetching data for {symbol} from {self.start_date} to {self.end_date} ({self.interval})")
            data_df = yf.download(
                tickers=symbol,
            ...
            if data_df.empty:
                print(f"YahooFinanceDataHandler: No data returned by yfinance for symbol {symbol}.")
                return pd.DataFrame(columns=['Open', 'High', 'Low', 'Close', 'Volume'])

            rename_map = {'Open': 'Open', 'High': 'High', 'Low': 'Low',
                          'Close': 'OriginalClose', 'Adj Close': 'Close', 'Volume': 'Volume'}
            ...
            data_df.index.name = 'Timestamp' # yfinance index is usually DatetimeIndex already
            
            # Only drop rows where Close price is NaN (most critical column)
            return data_df.dropna(subset=['Close'])

        except Exception as e:
            print(f"YahooFinanceDataHandler: Error fetching or processing data for {symbol}: {e}")
            return pd.DataFrame(columns=['Open', 'High', 'Low', 'Close', 'Volume'])
    ...
    def get_historical_data(self,
        ...
        df.index = pd.to_datetime(df.index.values, utc=True).tz_convert(None) if df.index.tz is not None else pd.to_datetime(df.index.values)
    ...
    def _prepare_data_iterator(self) -> Iterator[Tuple[pd.Timestamp, str, Dict[str, Any]]]:
        ...
        if not all_dfs:
            ...
            if not isinstance(self._combined_data_for_iter.index, pd.DatetimeIndex):
                 self._combined_data_for_iter.index = pd.to_datetime(self._combined_data_for_iter.index.values)
