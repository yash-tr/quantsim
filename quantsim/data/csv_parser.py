...
class CSVDataManager(DataHandler):
    ...
    def __init__(self,
        ...
        standard_names_map = {'Timestamp': 'Timestamp', 'Open': 'Open', 'High': 'High', 'Low': 'Low', 'Close': 'Close', 'Volume': 'Volume'}
        ...
        if self.date_column_in_csv not in effective_map and self.date_column_in_csv != 'Timestamp':
        ...
        for std_key, std_val in standard_names_map.items():
            if std_key not in effective_map and std_key not in effective_map.values() : # if neither key nor value is user-mapped
    ...
    def _load_and_process_data(self) -> pd.DataFrame:
        ...
        try:
            ...
            if self.date_column_in_csv in current_rename_map and current_rename_map[self.date_column_in_csv] == 'Timestamp':
            ...
            elif 'Timestamp' in df.columns and self.date_column_in_csv not in df.columns:
            ...
            df = df.rename(columns={k: v for k, v in current_rename_map.items() if k in df.columns})
            ...
            if col_to_parse_as_date in df.columns :
                 ...
                 if col_to_parse_as_date != 'Timestamp' and col_to_parse_as_date in df.columns:
                     df = df.drop(columns=[col_to_parse_as_date], errors='ignore')
            elif 'Timestamp' in df.columns and not isinstance(df.index, pd.DatetimeIndex): # If 'Timestamp' column exists (not index yet)
                df['Timestamp'] = pd.to_datetime(df['Timestamp'])
                df = df.set_index('Timestamp')
            elif not isinstance(df.index, pd.DatetimeIndex): # If index is not datetime but no obvious date column found
                 raise ValueError(f"Date column '{self.date_column_in_csv}' (or 'Timestamp') not found or index not DatetimeIndex.")
            ...
            for col in final_cols_no_ts:
                if col not in df.columns:
                    print(f"Warning: Standard column '{col}' for {self.symbol} not found after processing. Filling with NaN.")
                    df[col] = float('nan')
            ...
            print(f"Error loading/processing CSV for {self.symbol} from '{self.csv_file_path}': {e}")
            return pd.DataFrame(columns=['Open', 'High', 'Low', 'Close', 'Volume'])
    ...
    def get_historical_data(self,
        ...
        if symbol != self.symbol:
            print(f"Warning: CSVDataManager for {self.symbol} received request for {symbol}. No data returned.")
            return pd.DataFrame()

        df_to_return = self.data_frame
        if start_date: df_to_return = df_to_return[df_to_return.index >= pd.to_datetime(start_date)]
        if end_date: df_to_return = df_to_return[df_to_return.index <= pd.to_datetime(end_date)]
    ...
    def get_latest_bar(self, symbol: str) -> Optional[Tuple[pd.Timestamp, Dict[str, Any]]]:
        ...
        """
        if symbol != self.symbol or self.data_frame.empty: return None
    ...
    def get_latest_bars(self, symbol: str, n: int = 1) -> Optional[List[Tuple[pd.Timestamp, Dict[str, Any]]]]:
        ...
        """
        if symbol != self.symbol or self.data_frame.empty or n < 0: return None # n<0 is invalid
        if n == 0: return []

        n_to_fetch = min(n, len(self.data_frame))
        if n_to_fetch == 0: return []

        return [(idx, row.to_dict()) for idx, row in self.data_frame.iloc[-n_to_fetch:].iterrows()]
    ...
    def _prepare_data_iterator(self) -> Iterator[Tuple[pd.Timestamp, str, Dict[str, Any]]]:
        ...
        for timestamp, row in self.data_frame.iterrows():
            ...
            ohlcv_dict['Volume'] = int(ohlcv_dict.get('Volume', 0)) # Ensure Volume is int
    ...
    def continue_backtest(self) -> bool:
        ...
        """
        return not self.data_frame.empty and self._iter_current_idx < len(self.data_frame)

def load_csv_data(symbol: str, csv_file_path: str, date_column: str = 'Date',
    ...
    """
    manager = CSVDataManager(symbol=symbol, csv_file_path=csv_file_path,
                             date_column=date_column, column_map=column_map)
