...
class RiskParityPortfolio(Portfolio):
    ...
    """
    def __init__(self, initial_cash: float, event_queue, position_sizer=None, 
                 symbols: List[str] = None, lookback_period: int = 252, 
                 rebalance_on_close: bool = True, **kwargs):
        # Extract symbols before calling parent constructor
        self.symbols = symbols if symbols is not None else []
        
        # Call parent constructor with only the parameters it accepts
        super().__init__(initial_cash=initial_cash, event_queue=event_queue, 
                        position_sizer=position_sizer)
        
        # Initialize risk parity specific attributes
        self.lookback_period = lookback_period
        self.returns_history: Dict[str, List[float]] = {symbol: [] for symbol in self.symbols}
    ...
    def _calculate_risk_parity_weights(self) -> Optional[Dict[str, float]]:
        ...
        if not all(len(self.returns_history[symbol]) >= self.lookback_period 
        ...
        def objective(weights):
            ...
            risk_contributions = (weights * (cov_matrix @ weights)) / np.sqrt(portfolio_variance)
            return np.sum((risk_contributions - risk_contributions.mean())**2)

        num_assets = len(self.symbols)
        initial_weights = np.array([1/num_assets] * num_assets)
        bounds = tuple((0.001, 0.999) for _ in range(num_assets))  # Small bounds to avoid edge cases
