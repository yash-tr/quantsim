...
class Portfolio:
    ...
    """
    def __init__(self, initial_cash: float, event_queue: EventQueue, position_sizer: Optional[PositionSizer] = None):
        ...
        self.position_sizer = position_sizer if position_sizer is not None else FixedQuantitySizer()
    ...
    def on_fill(self, event: FillEvent) -> None:
        ...
        if event.symbol not in self.positions:
            position_quantity = event.quantity if event.direction == 'BUY' else -event.quantity
            ...
            transaction_quantity = event.quantity if event.direction == 'BUY' else -event.quantity
    ...
    def calculate_daily_returns(self) -> None:
        ...
        equity_df = pd.DataFrame(self.equity_curve, columns=['Timestamp', 'PortfolioValue'])
        equity_df = equity_df.set_index('Timestamp')
        self._daily_returns = equity_df['PortfolioValue'].resample('D').last().pct_change().dropna()
    ...
    def _calculate_metrics(self, risk_free_rate: float = 0.02) -> Dict:
        ...
        try:
            ...
            returns = self._daily_returns if self._daily_returns is not None else pd.Series(dtype=float)
            ...
            equity_series = pd.Series(equity_values, index=[t for t, v in self.equity_curve])
            ...
            if len(returns) > 0 and returns.std() > 0:
                ...
                sharpe_ratio = (mean_daily_return - daily_risk_free_rate) / daily_volatility * (252 ** 0.5) if daily_volatility > 0 else 0
                ...
                if time_delta_years == 0:
                    ...
                    equity_df = pd.DataFrame(self.equity_curve, columns=['Timestamp', 'PortfolioValue']).set_index('Timestamp')
                    returns_for_cagr = equity_df['PortfolioValue'].pct_change().dropna()
                    cagr = returns_for_cagr.sum() if len(returns_for_cagr) > 0 else 0
                else:
                    cagr = ((self.current_total_value / self.initial_cash) ** (1.0 / time_delta_years) - 1)
            else:
                annualized_return = 0
                annualized_volatility = 0
                sharpe_ratio = float('nan') if len(returns) > 0 and returns.std() == 0 else 0
                cagr = 0
            ...
            base_metrics = {
                ...
                'realized_pnl': sum(t.net_pnl for t in completed_trades) if completed_trades else 0.0,
    ...
    def calculate_sortino_ratio(self, returns: pd.Series, risk_free_rate: float) -> float:
    ...
    def calculate_calmar_ratio(self, returns: pd.Series, risk_free_rate: float) -> float:
    ...
    def export_trade_log(self, filepath: str, include_open_trades: bool = False) -> None:
    ...
    def print_final_summary(self) -> None:
        ...
        print(f"Total Return %: {((self.current_total_value - self.initial_cash) / self.initial_cash) * 100:.2f}%")
        
        if self.positions:
            print(f"\nOpen Positions:")
            for symbol, position in self.positions.items():
                print(f"  {symbol}: {position.quantity:.2f} shares @ ${position.last_price:.2f} (Unrealized P&L: ${position.unrealized_pnl:.2f})")
