...
def plot_equity_curve(equity_curve_df: pd.DataFrame,
    ...
    if 'PortfolioValue' not in equity_curve_df.columns:
        print("Plotter: 'PortfolioValue' column not found in equity_curve_df. Cannot plot.")
        return

    plt.figure(figsize=(12, 7))
    plt.plot(equity_curve_df.index, equity_curve_df['PortfolioValue'], label='Portfolio Value', color='blue')

    if initial_capital is not None:
        plt.axhline(y=initial_capital, color='grey', linestyle='--', label=f'Initial Capital (${initial_capital:,.0f})')
...
def plot_drawdown_series(drawdown_series: pd.Series,
                         output_path: str,
    ...
    plt.plot(drawdown_series.index, drawdown_series * 100, label='Drawdown', color='red')
    ...
    formatter = mticker.PercentFormatter(xmax=1.0, decimals=0) # xmax=1.0 if input is 0 to -1, or 100.0 if input is 0 to -100
