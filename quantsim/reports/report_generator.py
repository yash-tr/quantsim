...
class ReportGenerator:
    ...
    def generate_report(self, filename_prefix: str = "backtest_report"):
        ...
        report_filename = f"{filename_prefix}_{self.symbol}_{self.strategy_name}_{timestamp_str}.md"
        report_filepath = os.path.join(self.output_dir, report_filename)

        # --- Equity Curve ---
        equity_img_filename = f"equity_curve_{self.symbol}_{self.strategy_name}_{timestamp_str}.png"
        ...
        md_content.append(f"Date Generated: {timestamp_str}\n") # Use consistent timestamp
        ...
        if self.drawdown_series is not None and not self.drawdown_series.empty:
            ...
            drawdown_plot_img_path = os.path.join(self.output_dir, drawdown_img_filename)
        ...
        if self.portfolio_metrics and 'error' not in self.portfolio_metrics:
            metric_display_order = [
                'total_return_pct', 'cagr_pct', 'annualized_volatility_pct', 'sharpe_ratio', 'max_drawdown_pct',
                'realized_pnl', 'total_trades', 'num_winning_trades', 'num_losing_trades', 'win_rate_pct',
                'gross_profit', 'gross_loss', 'profit_factor', 'avg_win_pnl', 'avg_loss_pnl', 'avg_trade_duration_seconds'
            ]
            for key in metric_display_order:
                if key in self.portfolio_metrics:
                    ...
                    if isinstance(value, (float, np.floating)) and not pd.isna(value) :
                        if value == np.inf: formatted_value = "inf"
                        elif value == -np.inf: formatted_value = "-inf"
            ...
            md_content.append(f"Metrics calculation error: {self.portfolio_metrics['error']}\n")
        else:
        ...
        md_content.append(f"Detailed trade log saved to: '{self.symbol}_{self.strategy_name}_trades.csv' (in the same output directory).\n")
