...
class FeatureGenerator:
    ...
    def _add_lagged_returns(self, lags=5):
        ...
        for lag in range(1, lags + 1):
            self.data[f'lag_{lag}'] = self.data.groupby('symbol')['log_returns'].shift(lag)
        ...
        for feature_name in feature_list:
            if feature_name in feature_map:
                ...
                if feature_name not in self.data.columns:
                    print(f"Warning: Feature '{feature_name}' is not a known generation method or existing column.")
