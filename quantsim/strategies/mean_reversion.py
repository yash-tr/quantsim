...
from quantsim.core.events import MarketEvent, FillEvent, SignalEvent # Added SignalEvent for on_signal
...
class MeanReversionStrategy(Strategy):
    ...
    def __init__(self,
        ...
        self.current_prices: Dict[str, Optional[float]] = {sym: None for sym in self.symbols}
        self.sma_series: Dict[str, Optional[pd.Series]] = {sym: None for sym in self.symbols}
        self.atr_series: Dict[str, Optional[pd.Series]] = {sym: None for sym in self.symbols}

        self.current_positions: Dict[str, Optional[str]] = {sym: None for sym in self.symbols}
        self.last_order_direction: Dict[str, Optional[str]] = {sym: None for sym in self.symbols}
        ...
        if self.data_handler is not None:
            for sym in self.symbols:
                try:
                    ...
                    if isinstance(self.data_handler, pd.DataFrame): # Single DF from CLI or test
                        # For single DataFrame, use it for all symbols (common in tests)
                        symbol_df = self.data_handler
                    elif hasattr(self.data_handler, 'get_historical_data'): # Full DataHandler
                        symbol_df = self.data_handler.get_historical_data(sym)

                    if symbol_df is not None and not symbol_df.empty:
                        if 'Close' in symbol_df.columns:
                            self.sma_series[sym] = calculate_sma(symbol_df['Close'], self.sma_window)
                        else: print(f"Warning: MeanReversionStrategy ({sym}) SMA calc: 'Close' column missing.")

                        if self.atr_period > 0:
                            if all(col in symbol_df.columns for col in ['High', 'Low', 'Close']):
                                self.atr_series[sym] = calculate_atr(symbol_df['High'], symbol_df['Low'], symbol_df['Close'], self.atr_period)
                            else: print(f"Warning: MeanReversionStrategy ({sym}) ATR calc: HLC columns missing.")
                    elif symbol_df is None and not isinstance(self.data_handler, pd.DataFrame):
                        print(f"MeanReversionStrategy ({sym}): No data from data_handler for pre-calc.")
                except Exception as e:
                    print(f"MeanReversionStrategy ({sym}): Error pre-calculating indicators: {e}")

        print(f"MeanReversionStrategy initialized for {self.symbols}, SMA: {self.sma_window}, Threshold: {self.reversion_threshold:.2%}, ATR: {self.atr_period if self.atr_period > 0 else 'N/A'}")
    ...
    def on_market_data(self, event: MarketEvent) -> None:
        ...
        """
        if event.symbol not in self.symbols: return
        ...
        if symbol_sma_s is not None:
            ...
            if pd.isna(current_sma): current_sma = None

        if current_sma is None: return # Not enough data for SMA or SMA series not available
        ...
        deviation = (event.close - current_sma) / current_sma if current_sma != 0 else 0.0
        ...
        # Entry Logic
        if deviation < -self.reversion_threshold and last_dir != 'BUY': # Avoid re-buying if already in buy mode
            order_direction = 'BUY'
            if self.limit_order_offset_pct > 0:
                order_type = 'LMT'
                limit_price = event.close * (1 - self.limit_order_offset_pct)
        elif deviation > self.reversion_threshold and last_dir != 'SELL': # Avoid re-selling if already in sell mode
        ...
        elif current_pos == 'LONG' and deviation >= -self.reversion_threshold / 2 and last_dir != 'SELL_EXIT':
            order_direction = 'SELL'
            order_type = 'MKT'
            limit_price = None
            self.last_order_direction[event.symbol] = 'SELL_EXIT'
        elif current_pos == 'SHORT' and deviation <= self.reversion_threshold / 2 and last_dir != 'BUY_EXIT':
            order_direction = 'BUY'
            order_type = 'MKT'
            limit_price = None
        ...
        if order_direction:
            self._generate_order(
                symbol=event.symbol, order_type=order_type, direction=order_direction,
            ...
            print(f"{event.timestamp} MeanReversionStrategy ({event.symbol}): Dev: {deviation:.2%}. Generated {order_direction} {order_type} order. RefPx: {event.close:.2f}" + (f", LimPx: {str(limit_price) if limit_price is not None else 'N/A'}"))
    ...
    def on_fill(self, event: FillEvent) -> None:
        ...
        if event.direction == 'BUY':
            if last_order_dir_before_fill == 'BUY_EXIT': # This was an exit of a short
                ...
                if current_intended_pos_before_fill != 'LONG': is_opening_trade = True
            ...
            if last_order_dir_before_fill == 'SELL_EXIT': # This was an exit of a long
                ...
                if current_intended_pos_before_fill != 'SHORT': is_opening_trade = True
                self.current_positions[event.symbol] = 'SHORT'
                # self.last_order_direction[event.symbol] = 'SELL' # Set by on_market_data

        print(f"MeanReversionStrategy ({event.symbol}): Fill {event.direction} {event.quantity} @ {event.fill_price}. Intended Pos: {self.current_positions.get(event.symbol)}")
        ...
        if self.stop_loss_pct > 0 and is_opening_trade:
            ...
            if stop_price is not None and stop_direction is not None:
                ...
                print(f"MeanReversionStrategy ({event.symbol}): Issuing STP {stop_direction}, Qty: {event.quantity}, StopPx: {stop_price:.2f}")
                self._generate_order(
                    symbol=event.symbol, order_type='STP', quantity=event.quantity,
                    direction=stop_direction, timestamp=event.timestamp,
                    reference_price=event.fill_price,
                    stop_price=stop_price, current_atr=current_atr_val
                )
