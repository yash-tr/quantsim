...
class MomentumStrategy(Strategy):
        ...
        if self.atr_period > 0 and self.data_handler is not None:
            for sym_for_atr in self.symbols:
                try:
                    ...
                    if isinstance(self.data_handler, pd.DataFrame): # Single DataFrame for all symbols (common in tests)
                        symbol_df = self.data_handler
                    elif hasattr(self.data_handler, 'get_historical_data'): # Full DataHandler object
                    ...
                    if symbol_df is not None and not symbol_df.empty:
                        if all(col in symbol_df.columns for col in ['High', 'Low', 'Close']):
                            self.atr_series[sym_for_atr] = calculate_atr(
                                symbol_df['High'], symbol_df['Low'], symbol_df['Close'], self.atr_period
                            )
                        else:
                             print(f"Warning: MomentumStrategy ({sym_for_atr}) ATR calc: HLC columns missing for ATR.")
                    elif symbol_df is None and not isinstance(self.data_handler, pd.DataFrame) :
                         print(f"MomentumStrategy ({sym_for_atr}): No data from data_handler for ATR pre-calc.")
                except Exception as e:
                    print(f"MomentumStrategy ({sym_for_atr}): Error pre-calculating ATR: {e}")

        print(f"MomentumStrategy initialized for {self.symbols}, window: {self.momentum_window}, ATR: {self.atr_period if self.atr_period > 0 else 'N/A'}.")
    ...
    def on_market_data(self, event: MarketEvent) -> None:
        ...
        if len(self.prices[event.symbol]) < self.momentum_window: return

        price_now = self.prices[event.symbol][-1]
        price_then = self.prices[event.symbol][-self.momentum_window] # Price N periods ago
        ...
        if symbol_atr_s is not None:
            current_atr_val = symbol_atr_s.get(event.timestamp) # Get ATR value for specific timestamp
            if pd.isna(current_atr_val): current_atr_val = None # Ensure None if pandas returns NaN
        ...
        elif momentum_return < 0 and self.last_order_direction.get(event.symbol) != 'SELL':
        ...
        if order_direction:
            ...
            self._generate_order(symbol=event.symbol, order_type=order_type, direction=order_direction,
            ...
            print(f"{event.timestamp} MomentumStrategy ({event.symbol}): Momentum {momentum_return:.2%}. Generated {order_direction} {order_type} order. RefPx: {event.close:.2f}" + (f", LimPx: {str(limit_price) if limit_price is not None else 'N/A'}"))
    ...
    def on_fill(self, event: FillEvent) -> None:
        ...
        is_opening_trade = False # True if this fill opens a new position or changes direction
        ...
        print(f"MomentumStrategy ({event.symbol}): Fill {event.direction} {event.quantity} @ {event.fill_price}. Intended Pos: {self.current_positions.get(event.symbol)}")
        ...
        if self.stop_loss_pct > 0 and is_opening_trade:
            ...
            if stop_price is not None and stop_direction is not None:
                ...
                print(f"MomentumStrategy ({event.symbol}): Issuing STP {stop_direction}, Qty: {event.quantity}, StopPx: {stop_price:.2f}")
